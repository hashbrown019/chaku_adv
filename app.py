...
def advadmin():
	...
	return render_template("adv_page.html",ads=ads, user = session['user_log'])


@app.route("/post_advertisement")
@adver_auth()
def post_advertisement():
	return render_template("create_adds.html",user=session['user_log'])
...
def admin():
	...
	return render_template("_home.html",completed_add=(len(ads_logs)-skip_ad)*1,ad_views=len(ads_logs),total_call=len(logs))
...
def ads():
	...
	return render_template("_adds.html",ads=ads,advertisers=adver)
...
@admin_auth_()
def new_advertisers():
	return render_template("_add_advertizer.html",user=session['admin_log'])
...
def adlogs():
	...
	_USERS = firebase.get_data("users",True)
	return render_template("_ads_logs.html",logs = _LOGS, users= _USERS,advertisments=ads)
